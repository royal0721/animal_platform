...
export class MapSheetComponent implements OnInit {
      ...
      .subscribe((informs: any[]) => {
        ...
        google.maps.event.addListener(map, 'click', function (event) {
          ...
          if (times <= 1) {
            ...
            (<HTMLInputElement>(
              document.getElementById('location_added_long')
            )).value = event.latLng;
            console.log(
              (<HTMLInputElement>document.getElementById('location_added_long'))
                .value,
            );
          } else if (times > 1) {
            placeMarker(event.latLng);
                ...
                if (results[0]) {
                  ...
                  for (
                    ...
                    if (results[0].address_components[i].types[0] == 'route') {
                      ...
                      (<HTMLSelectElement>(
                        document.getElementById('location_added_addr')
                      )).value = road;
                    } else if (
                      ...
                      (<HTMLSelectElement>(
                        document.getElementById('address2_selector')
                      )).value = area;
                    } else if (
                      results[0].address_components[i].types[0] ==
                      ...
                      (<HTMLSelectElement>(
                        document.getElementById('address_selector')
                      )).value = city;
                    }
                  }
                  ...
                  const addcontent =
                    ...
                    '預計通報位置:</div><div style="font-size:18px;color:rgb(93,93,93);">' +
                    address.slice(5) +
                    '</div></div>';
    ...
    function convertToMapPoints(response) {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        var contentString =
          ...
          '<h5 style="background-color:grey;color:white;text-align: center;">' +
          '編號: ' +
          animal.id +
          '</h5>' +
          '<br><div><h6 style="font-weight:bold;color:#c1c1c1;">性別/品種:</h6><b style="padding-left:5px;font-weight:bold;color:grey;">' +
          gender +
          animal.type +
          '</b></div>' +
          '<br><div><h6 style="font-weight:bold;color:#c1c1c1;">名字:</h6>' +
          '<b style="padding-left:5px;font-weight:bold;color:grey;">' +
          animal.name +
          '</b></div>' +
          '<br><div><h6 style="font-weight:bold;color:#c1c1c1;">地址:</h6><b style="padding-left:5px;font-weight:bold;color:grey;"> ' +
          animal.address +
          animal.address2 +
          animal.address3 +
          '</b></div>' +
          '<br><div><h6 style="font-weight:bold;color:#c1c1c1;">經緯度:</h6><b style="padding-left:5px;font-weight:bold;color:grey;"> ' +
          animal.latlong +
          '</b></div>' +
          '<img style="width:170px;height:auto;padding-top:10px;" src="data:image/png;base64, ' +
          animal.img_url +
          '" />';
        '</p>' + '</div>';
  ...
  click_to_target() {
    ...
    if (
      (<HTMLSelectElement>document.getElementById('address_selector')).value &&
      (<HTMLSelectElement>document.getElementById('address2_selector')).value
    ) {
      console.log(
        (<HTMLSelectElement>document.getElementById('address_selector')).value,
      );
