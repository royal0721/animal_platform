...
export class InformSheetComponent {
  informs: any[] = [];
  ...
  userForm = new UntypedFormGroup({
    informer_id: new UntypedFormControl(
      { value: '', disabled: true },
    ...
    latlong: new UntypedFormControl(
      { value: '', disabled: true },
      Validators.compose([Validators.required]),
    ),
  });
  ...
  onSubmit() {
    ...
    this.userForm.controls['address3'].setValue(
      (<HTMLSelectElement>document.getElementById('location_added_addr')).value,
    );
    this.userForm.controls['latlong'].setValue(
      (<HTMLSelectElement>document.getElementById('location_added_long')).value,
    );
